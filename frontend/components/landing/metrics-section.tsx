...
const metrics = [
  { 
    ...
    sublabel: "Seed ledger plus uploads"
  },
  { 
    value: 4, 
    suffix: "", 
    label: "Agent identities",
    sublabel: "Receipt, category, report, payment"
  },
  { 
    value: 100, 
    suffix: "%", 
    label: "Protected actions",
    sublabel: "Audit row generated"
  },
  { 
    value: 0, 
    suffix: "", 
    label: "Secret leaks",
    sublabel: "No raw keys in the browser"
  },
];
    ...
    <section id="metrics" className="relative py-32 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-16">
          <div>
            <p className="text-sm font-mono text-primary mb-3">{"// LIVE PROOF"}</p>
            <h2 className="text-3xl lg:text-5xl font-semibold tracking-tight text-balance">
              Agent operations<br />with proof.
            </h2>
          </div>
          <div className="flex items-center gap-3 font-mono text-sm text-muted-foreground">
            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
            <span>Terminal3 adapter online</span>
        ...
        <div className="mt-12 p-6 rounded-xl bg-card border border-border card-shadow">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-mono text-sm text-muted-foreground">Live protected-action feed</span>
          </div>
          <div className="font-mono text-xs space-y-2 text-muted-foreground overflow-hidden h-24">
            <ActivityLine time="now" event="POST /api/chat" region="category-agent" status="200" latency="proof" />
            <ActivityLine time="1s" event="POST /api/receipts" region="receipt-agent" status="200" latency="digest" />
            <ActivityLine time="2s" event="POST /api/reports" region="report-agent" status="200" latency="signed" />
            <ActivityLine time="3s" event="POST /api/actions/protected" region="payment-agent" status="202" latency="approval" />
          </div>
        </div>
      </div>
    </section>
